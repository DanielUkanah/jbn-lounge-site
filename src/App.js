...
function App() {
  ...
  const renderHome = () => (
    <div className="reveal">
      <section id="hero" style={{ ...styles.hero, overflow: 'hidden' }}>
        {/* Background layer with Ken Burns zoom effect */}
        <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 0 }}>
            <img src="/images/hero-bg.png" alt="Hero" className="hero-bg-zoom" onError={(e) => { e.target.style.display = 'none' }} />
        </div>
      ...
      <div className="content-wrapper" style={styles.contentWrapper}>
        <section className="section reveal" style={styles.section}>
          <h2 className="section-title reveal" style={styles.sectionTitle}>Today's Specials</h2>
          <div className="specials-grid" style={styles.specialsGrid}>
            {specials.map(item => (
              <div key={item.id} className="special-card reveal" style={styles.specialCard}>
        ...
        <section className="section reveal" style={{ ...styles.section, backgroundColor: 'rgba(26,67,45,0.1)', padding: '40px', borderRadius: '12px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '40px' }}>
          <div style={{ flex: '1', minWidth: '280px' }} className="special-card">
            <img 
          ...
          <div style={{ flex: '1', minWidth: '280px' }}>
            <h2 className="section-title reveal" style={{ fontSize: '2.5rem', marginBottom: '20px', color: 'var(--gold)', fontFamily: 'Playfair Display' }}>Championship Snooker</h2>
            <p className="reveal" style={{ fontSize: '1.1rem', color: '#D8CBB8', marginBottom: '30px', lineHeight: '1.6' }}>
            ...
            <button className="btn-gold reveal" onClick={() => navigateTo('snooker')}>Reserve Your Table Now</button>
          </div>
        </section>
        ...
        <section className="section reveal" style={styles.section}>
          <div className="features-grid" style={styles.featuresGrid}>
            {[
              ...
              <div key={idx} className="feature-box reveal" style={styles.featureBox}>
  ...
  const renderEvents = () => (
    <div className="reveal" style={{ padding: '120px 20px', textAlign: 'center', minHeight: '60vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
  ...
  const renderGallery = () => (
    <div className="content-wrapper reveal" style={{ padding: '80px 50px', maxWidth: '1400px', margin: '0 auto', minHeight: '60vh' }}>
      ...
      <div className="features-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
        {[1, 2, 3, 4, 5, 6].map(i => (
          <div key={i} className="feature-box reveal" style={{ backgroundColor: 'rgba(255,255,255,0.05)', height: '250px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#D8CBB8', border: '1px solid rgba(197, 160, 89, 0.2)' }}>
            [ Placeholder Image {i} ]
          </div>
        ))}
      </div>
    ...
    <div className="app-container">
      ...
      <nav className="app-nav" style={styles.nav}>
        <div style={styles.logo} onClick={() => navigateTo('home')}>
          <img src="/images/logo.jpg" alt="JBN Lounge" style={{ height: '55px', mixBlendMode: 'lighten', cursor: 'pointer' }} />
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          <button className="mobile-menu-btn" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} style={{ background: 'none', border: 'none', color: 'var(--gold)', cursor: 'pointer', padding: 0 }}>
      ...
      {/* --- MOBILE MENU --- */}
      {isMobileMenuOpen && (
        <div className="mobile-menu-dropdown reveal visible" style={styles.mobileDropdown}>
      ...
      <main>
        ...
        {currentView === 'menu' && (
          <div className="content-wrapper reveal" style={styles.contentWrapper}>
             <FoodMenu addToCart={addToCart} />
          </div>
        )}
        {currentView === 'snooker' && (
          <div className="content-wrapper reveal" style={{ ...styles.contentWrapper, paddingTop: '40px', paddingBottom: '80px' }}>
             <SnookerLounge />
          </div>
        )}
      ...
      <a 
        ...
        style={{ position: 'fixed', bottom: '30px', right: '30px', backgroundColor: '#25D366', color: 'white', width: '60px', height: '60px', borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.3)', zIndex: 9999, cursor: 'pointer', textDecoration: 'none', transition: 'transform 0.3s ease' }}
      ...
      {/* --- FOOTER --- */}
      <footer className="footer reveal" style={styles.footer}>
        <div className="footer-grid" style={styles.footerGrid}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            ...
            <div style={{ width: '100%', height: '150px', borderRadius: '8px', overflow: 'hidden', border: '1px solid rgba(197, 160, 89, 0.3)' }}>
              <iframe 
                src="https://maps.google.com/maps?q=Rumuekini,%20Port%20Harcourt&t=&z=14&ie=UTF8&iwloc=&output=embed" width="100%" height="100%" style={{ border: 0 }} allowFullScreen="" loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="JBN Lounge Location">
              </iframe>
            </div>
          </div>
...
const styles = {
  ...
  hero: { position: 'relative', minHeight: '80vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 20px', backgroundColor: 'var(--mahogany)' },
  ...
  specialCard: { backgroundColor: 'var(--beige)', borderRadius: '8px' },
  specialImg: { width: '100%', height: '100%', objectFit: 'cover' },
  
  featuresGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' },
  featureBox: { border: '1px solid var(--gold)', borderRadius: '8px', padding: '40px 20px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' },
